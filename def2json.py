...
def find_alpha_bbox_tiles(img: Image.Image, tile_size: int = TILE_SIZE):
    pixels = img.load()
    min_x = img.width
    min_y = img.height
    max_x = -1
    ...
    tile_left = min_x // tile_size
    tile_top = min_y // tile_size
    tile_right = max_x // tile_size
    tile_bottom = max_y // tile_size
    return tile_left, tile_top, tile_right, tile_bottom
...
def generate_vcmi_template_from_png(png_path, animation_name=None):
    ...
    stem = Path(png_path).stem.upper()
    object_name = (animation_name or stem).upper()

    return {
        object_name: {
            "animation": f"{object_name}.def",
            "editorAnimation": f"{object_name}.def",
...
def export_frame_images(d, filetype, output_dir, only_config=False, ignore_filename=False, ignore_group=False, merge_shadow=False, overlay_from_colours=False):
    images = []
    generated_pngs = []
            ...
            try:
                if not only_config:
                    if filetype == "def":
                        ...
                        shadow_img = d.read_image("shadow", group, frame)
                        overlay_img = d.read_image("overlay", group, frame)

                        png_path = os.path.join(output_dir, f"{image_name}.png")
                        if merge_shadow and normal_img and shadow_img:
                            combined = Image.alpha_composite(normal_img, shadow_img)
                            ...
                            if normal_img:
                                ...
                                generated_pngs.append(png_path)
                            if shadow_img and not merge_shadow:
...
def process_def_file(path, filetype, only_config=False, ignore_filename=False, ignore_group=False, merge_shadow=False, overlay_from_colours=False, vcmi_template=False, template_frame=0, mask_only=False):
    ...
    if mask_only and not vcmi_template:
        raise ValueError("--maskonly requires --vcmi-template")
    if mask_only and only_config:
        raise ValueError("--maskonly cannot be combined with --onlyconfig")
    ...
    try:
        with deffile.open(path) as d:
            images, generated_pngs = export_frame_images(
                d,
                filetype,
                export_dir,
        ...
        if vcmi_template:
            if not generated_pngs:
                ...
                if template_frame < 0 or template_frame >= len(generated_pngs):
                    raise ValueError(f"template_frame {template_frame} is out of range, exported PNG count is {len(generated_pngs)}")

                selected_png = generated_pngs[template_frame]
                template = generate_vcmi_template_from_png(selected_png, animation_name=stem.upper())
...
def build_argument_parser():
    ...
    parser.add_argument("--template-frame", type=int, default=0, help="Frame index to use for VCMI template generation.")
